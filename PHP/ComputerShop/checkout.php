<?php # checkout.php
	...
	$q = "INSERT INTO orders (customer_id, total) VALUE ($cid, $total)";
	if (mysqli_affected_rows($dbc) == 1) {
		...
		// Prepare the query:
		$q = "INSERT INTO order_conents (order_id, print_id, quantity, price) VALUES (?, ?, ?, ?)";
		$stmt = mysqli_prepare($dbc, $q);
		mysqli_stmt_bind_param($stmt, 'iiid', $iod, $pid, $qty, $price);
		
		// Execute each queryl count the total affected:
		$affected = 0;
